...
def deposito(request):
    # Obtém as contas configuradas no painel administrativo
    pix_accounts = PlatformBankDetails.objects.filter(type='PIX')
    usdt_wallets = PlatformBankDetails.objects.filter(type='USDT')
    
    # Extrai o primeiro registro para renderizar as chaves e dados no front-end
    pix_account = pix_accounts.first()
    usdt_wallet = usdt_wallets.first()
    
    settings = PlatformSettings.objects.first()
    deposit_instruction = settings.deposit_instruction if settings else ''
    
    # Captura os valores de depósitos reais dos seus Níveis do banco de dados
    level_deposits = Level.objects.all().values_list('deposit_value', flat=True).distinct().order_by('deposit_value')
    
    # Converte os valores para listas limpas de floats/inteiros para leitura direta do JavaScript JSON
    level_deposits_brl_list = [float(d) for d in level_deposits]
    level_deposits_usdt_list = [float(d) for d in level_deposits] 
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            deposit.status = 'pending'  # Garante o estado inicial como pendente antes da aprovação
            deposit.save()
    ...
    context = {
        ...
        'form': form,
        'level_deposits_brl_list': level_deposits_brl_list,
        'level_deposits_usdt_list': level_deposits_usdt_list,
        'deposit_success': False,
        'deposit_instruction': deposit_instruction,
        
        # Mapeamento exato baseado na estrutura do painel administrativo (IBAN e bank_name)
        'pix_bank_name': pix_account.bank_name if pix_account else "Banco Não Configurado",
        'pix_holder_name': pix_account.account_holder_name if pix_account else "Titular Não Configurado",
        'pix_key': pix_account.IBAN if pix_account else "Chave Não Configurada",
        
        'usdt_network': usdt_wallet.bank_name if usdt_wallet else "Rede Não Configurada",
        'usdt_address': usdt_wallet.IBAN if usdt_wallet else "Carteira Não Configurada",
    }
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if not is_working_day:
                ...
                messages.error(request, 'Fora do horário de saque (Brazil: 09:00 às 17:00).')
...
def process_task(request):
    ...
    try:
        ...
        # Se não é VIP, verificamos se o estágio ainda é válido
            ...
            total_task_earnings = Decimal('2.00')
