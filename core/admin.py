        ...
        js = (
            ...
            (function($) {
                $(document).ready(function() {
                    function toggleLabels(typeValue) {
                        var $bankNameLabel = $(".field-bank_name label");
                        var $holderLabel = $(".field-account_holder_name label");
                        var $ibanLabel = $(".field-IBAN label");
                        
                        if (typeValue === "PIX") {
                            $bankNameLabel.text("Nome do Banco:");
                            $holderLabel.text("Nome do Titular da Conta:");
                            $ibanLabel.text("Chave PIX (E-mail, Telefone, CPF ou Aleatória):");
                            $(".field-account_holder_name").show();
                        } else if (typeValue === "USDT") {
                            $bankNameLabel.text("Rede / Canal de Transmissão (ex: TRC20, ERC20):");
                            $holderLabel.text("Titular / Identificação Opcional (USDT):");
                            $ibanLabel.text("Endereço da Carteira USDT (Hash completo):");
                        }
                    }

                    var $typeField = $("#id_type");
                    toggleLabels($typeField.val());

                    $typeField.change(function() {
                        toggleLabels($(this).val());
                    });
                });
            })(django.jQuery);
            ''',
